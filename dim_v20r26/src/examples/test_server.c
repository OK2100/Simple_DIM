...
//	}
	...
	sprintf(aux, "%s/Service_DimLong", srvname);
	dis_add_service(aux, "L", &testDimLong, sizeof(testDimLong),
		(void *)0, 0);
	sprintf(aux, "%s/Service_DimXLong", srvname);
	dis_add_service(aux, "X", &testDimXLong, sizeof(testDimXLong),
		(void *)0, 0);
	printf("Dim X size long %d\n", sizeof(long));
	printf("Dim X size longlong %d\n", sizeof(longlong));
	printf("Dim X size long long %d\n", sizeof(long long));
	/*
	big_buff[0] = 1;
	for(i = 0; i < 20; i++)
	{
